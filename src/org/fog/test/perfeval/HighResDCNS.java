...
public class HighResDCNS {
	...
	public static void main(String[] args) {
		...
		if (args.length >= 1) {
        	...
        	Resolution= Double.parseDouble(args[0]);
        }
	...
	private static Application createApplication(String appId1, int userId){
		...
		System.out.println("Resolution is "+scale);
		application.addAppEdge("CAMERA", "motion_detector", scale*1000, scale*20000, "CAMERA", Tuple.UP, AppEdge.SENSOR); // adding edge from CAMERA (sensor) to Motion Detector module carrying tuples of type CAMERA
		application.addAppEdge("motion_detector", "object_detector", scale*2000, scale*2000, "MOTION_VIDEO_STREAM", Tuple.UP, AppEdge.MODULE); // adding edge from Motion Detector to Object Detector module carrying tuples of type MOTION_VIDEO_STREAM
		application.addAppEdge("object_detector", "user_interface", scale*500, scale*2000, "DETECTED_OBJECT", Tuple.UP, AppEdge.MODULE); // adding edge from Object Detector to User Interface module carrying tuples of type DETECTED_OBJECT
		...
		//final AppLoop loop3 = new AppLoop(new ArrayList<String>(){{add("motion_detector");add("object_detector");add("object_tracker");add("PTZ_CONTROL");}});
