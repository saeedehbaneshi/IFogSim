...
public class ModulePlacementEdgewards extends ModulePlacement{
	...
	private List<String> shiftModuleNorth(String moduleName, double cpuLoad, Integer deviceId, List<String> operatorsToPlace) {
		...
		for(String module : modulesToShift){
			...
			//SaeedehCorrected:remove+1
			//moduleToNumInstances.put(module, getCurrentModuleInstanceNum().get(deviceId).get(module)+1);
			moduleToNumInstances.put(module, getCurrentModuleInstanceNum().get(deviceId).get(module));
			//SaeedehCorrected: multiply load to the number of instances
			totalCpuLoad += getCurrentModuleLoadMap().get(deviceId).get(module) * moduleToNumInstances.get(module);
		...
		//SaeedehCorrected: this line is not need anymore
		//loadMap.put(moduleName, loadMap.get(moduleName)+cpuLoad);
		//SaeedehCorrected: in addition to its load we should add one instance
		moduleToNumInstances.put(moduleName, moduleToNumInstances.get(moduleName)+1);
			...
			if(getCurrentCpuLoad().get(id) + totalCpuLoad > fogDevice.getHost().getTotalMips()){
				...
				for(String module : _modulesToShift){
					if(!modulesToShift.contains(module)){
						...
						totalCpuLoad += getCurrentModuleLoadMap().get(id).get(module) * moduleToNumInstances.get(module);
				...
				for(String module : loadMap.keySet()){
					//SaeedehCorrected: multiply by num instances
					totalLoad += loadMap.get(module) * moduleToNumInstances.get(module);
				...
				//SaeedehCorrected: Just consider the moduleName has placed
				//And the rest of modules that need to moved with that module do not considered as placed
				//operatorsToPlace.removeAll(loadMap.keySet());
				operatorsToPlace.remove(moduleName);
				List<String> placedOperators = new ArrayList<String>();
				//SaeedehCorrected: here also 
				//for(String op : loadMap.keySet())placedOperators.add(op);
				placedOperators.add(moduleName);
